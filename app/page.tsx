...
const categories = [
  { name: 'Papel', icon: FileText },
  { name: 'Plástico', icon: Wine },
  { name: 'Vidrio', icon: Wine },
  { name: 'Orgánico', icon: Leaf },
]

const results = [
  { name: 'Papel', value: 92, color: 'bg-[#d8bd7a]' },
  { name: 'Plástico', value: 5, color: 'bg-[#7290c6]' },
  { name: 'Vidrio', value: 2, color: 'bg-[#91b7a3]' },
  { name: 'Orgánico', value: 1, color: 'bg-[#a9896b]' },
]
      ...
      <div className="mx-auto flex min-h-[calc(100vh-3.5rem)] max-w-6xl flex-col">
        <header className="mb-8 flex items-start justify-between gap-5 border-b border-[#22396F]/80 pb-6">
          ...
          <div className="hidden items-center gap-2 rounded-full border border-[#22396F] px-3 py-2 text-xs text-[#b6c0da] sm:flex">
            <span className="size-2 rounded-full bg-[#91b7a3]" aria-hidden="true" /> Modelo listo
          </div>
        </header>
          ...
          <Card className="border-[#22396F] bg-[#0D1C42] text-[#FCF1D0] shadow-2xl shadow-black/20">
            ...
            <CardContent className="px-6 pb-7 pt-6 sm:px-8">
              <button type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop} className="group relative flex min-h-[280px] w-full flex-col items-center justify-center overflow-hidden rounded-2xl border border-dashed border-[#687cae] bg-[#010736]/45 p-6 text-center transition-colors hover:border-[#FCF1D0] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FCF1D0]">
                {preview ? (
                  <>
                    <img src={preview} alt="Vista previa del residuo cargado" className="absolute inset-0 size-full object-contain p-3" />
              ...
              <Button type="button" onClick={() => setAnalyzed(true)} disabled={!preview} className="mt-5 h-12 w-full rounded-xl bg-[#22396F] text-[#FCF1D0] shadow-lg shadow-[#010736]/40 hover:bg-[#2c4a8e] disabled:cursor-not-allowed disabled:opacity-50"><Sparkles data-icon="inline-start" />Analizar imagen<ArrowUp data-icon="inline-end" /></Button>
              <p className="mt-4 flex items-center justify-center gap-2 text-center text-[11px] text-[#8291ba]"><Check className="size-3.5 text-[#91b7a3]" aria-hidden="true" /> Sin conexión a modelo · resultado demostrativo</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-[#22396F] bg-[#0D1C42] text-[#FCF1D0] shadow-2xl shadow-black/20">
            ...
            <CardContent className="px-6 pb-7 pt-6 sm:px-8">
              <div className="mb-7 flex items-center gap-4 rounded-2xl border border-[#22396F] bg-[#010736]/50 p-4">
                <div className="flex size-14 items-center justify-center rounded-xl bg-[#22396F] text-[#d8bd7a]"><FileText className="size-7" strokeWidth={1.5} aria-hidden="true" /></div>
                <div><p className="text-[10px] uppercase tracking-[0.2em] text-[#8291ba]">Categoría detectada</p><p className="mt-1 text-2xl font-semibold">Papel</p></div>
                <div className="ml-auto text-right"><p className="font-mono text-2xl font-semibold text-[#d8bd7a]">92%</p><p className="text-[10px] text-[#8291ba]">confianza</p></div>
              </div>
              <div className="flex flex-col gap-5">
                {results.map(({ name, value, color }) => <div key={name}><div className="mb-2 flex justify-between text-xs"><span className={name === 'Papel' ? 'font-semibold' : 'text-[#b6c0da]'}>{name}</span><span className="font-mono text-[#b6c0da]">{value}%</span></div><Progress value={analyzed || name !== 'Papel' ? value : 0} className="gap-0"><ProgressTrack className="h-2 bg-[#010736]"><ProgressIndicator className={color} /></ProgressTrack></Progress></div>)}
              </div>
              <div className="mt-8 flex items-start gap-3 border-t border-[#22396F]/70 pt-5"><Recycle className="mt-0.5 size-4 shrink-0 text-[#91b7a3]" aria-hidden="true" /><p className="text-xs leading-relaxed text-[#9ba9cf]">Este residuo puede depositarse en el contenedor azul de papel y cartón.</p></div>
            </CardContent>
          </Card>
        </div>
        <footer className="mt-6 flex items-center justify-between text-[10px] uppercase tracking-[0.18em] text-[#66769f]"><span>EcoScan / Clasificación experimental</span><span className="hidden sm:inline">Teachable Machine · próximamente</span></footer>
      </div>
    </main>
  )
}
